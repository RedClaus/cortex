...
// renderSettings renders the settings panel for model selection
func (m Model) renderSettings() string {
	var sections []string

	// Title
	sections = append(sections, m.styles.Header.Render("Settings - Model Selection"))
	sections = append(sections, "")

	// Lane selector
	lanes := []string{"Fast", "Balanced", "Quality"}
	var laneDisplay []string
	for i, lane := range lanes {
		if i == m.settingsLane {
			laneDisplay = append(laneDisplay, m.styles.BotMsg.Render("["+lane+"]"))
		} else {
			laneDisplay = append(laneDisplay, " "+lane+" ")
		}
	}
	sections = append(sections, "Lane: "+strings.Join(laneDisplay, "  "))
	sections = append(sections, "")

	// Model list for current lane
	sections = append(sections, "Available Models:")
	sections = append(sections, "")

	for i, model := range m.settingsModels {
		prefix := "  "
		if i == m.settingsSelected {
			prefix = "> "
		}
		line := fmt.Sprintf("%s%s", prefix, model)
		if i == m.settingsSelected {
			sections = append(sections, m.styles.UserMsg.Render(line))
		} else {
	...
	// Help text
	sections = append(sections, m.styles.ToolStatus.Render("Use ←/→ to switch lanes, ↑/↓ to select model"))
	sections = append(sections, m.styles.ToolStatus.Render("Enter to confirm, Esc to cancel"))

	settingsContent := lipgloss.JoinVertical(lipgloss.Left, sections...)
